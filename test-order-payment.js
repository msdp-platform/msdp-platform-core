...
async function testServices() {
  ...
  try {
    ...
    let userToken;
    try {
      const registerResponse = await axios.post(`${USER_SERVICE}/api/auth/register`, testUser);
      console.log('✅ User registered successfully');
      console.log('   Response:', JSON.stringify(registerResponse.data, null, 2));
      userToken = registerResponse.data.token || registerResponse.data.data?.token;
    } catch (error) {
      if (error.response?.status === 409) {
        console.log('ℹ️  User already exists, logging in...');
        const loginResponse = await axios.post(`${USER_SERVICE}/api/auth/login`, {
