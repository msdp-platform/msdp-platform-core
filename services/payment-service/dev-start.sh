...
# Payment Service Fast Development Startup Script
set -e
...
echo "💳 Starting MSDP Payment Service (Payment Processing)"
echo "===================================================="
...
echo "🔨 Starting payment processing service..."
...
while [ $DB_RETRY_COUNT -lt $MAX_DB_RETRIES ]; do
    if docker exec msdp-payment-postgres-dev pg_isready -U msdp_user -d msdp_payments > /dev/null 2>&1; then
...
if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
    ...
    echo "   docker-compose -f docker-compose.dev.yml logs payment-service"
    exit 1
fi
...
echo "🗄️ PgAdmin: http://localhost:8089 (admin@msdp.com / admin123)"
echo ""
echo "🚀 Payment Service Functions:"
...
echo "  - POST /api/payments/calculate-total"
echo "  - GET  /api/payment-methods/user/{userId}"
echo "  - POST /api/payment-methods/add"
echo "  - GET  /api/transactions/user/{userId}"
echo ""
echo "🔧 Quick commands:"
echo "  logs:    docker-compose -f docker-compose.dev.yml logs -f"
echo "  stop:    docker-compose -f docker-compose.dev.yml down"
echo "  restart: docker-compose -f docker-compose.dev.yml restart $SERVICE_NAME"
echo "  db:      docker exec msdp-payment-postgres-dev psql -U msdp_user -d msdp_payments"
echo ""
