...
async function testCustomerFrontend() {
  ...
  try {
    ...
    try {
      const frontendResponse = await axios.get(`${CUSTOMER_FRONTEND}`, { timeout: 5000 });
      console.log('✅ Customer Frontend:', frontendResponse.status === 200 ? 'Available' : 'Issue');
    } catch (error) {
    ...
    try {
      const registerResponse = await axios.post(`${CUSTOMER_FRONTEND}/api/register`, testUser, {
      ...
      if (cookies && cookies.some(cookie => cookie.includes('msdp_session'))) {
      ...
      if (error.response?.status === 409) {
        ...
        // Test login
        const loginResponse = await axios.post(`${CUSTOMER_FRONTEND}/api/login`, {
